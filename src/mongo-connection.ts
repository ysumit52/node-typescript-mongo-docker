...
(<any>mongoose).Promise = globalThis.Promise;
...
export default class MongoConnection {
  ...
  public close(onClosed: (err: any) => void) {
    ...
    mongoose.connection
      .close()
      .then(() => onClosed(null)) // Pass null as there's no error on success
      .catch(err => onClosed(err)); // Pass the error if closing fails
  }
